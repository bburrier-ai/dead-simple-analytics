"""drop correlation_key; index session_id for grouping"""
...
def upgrade() -> None:
    op.execute("""
        ...
        CREATE INDEX idx_events_site_session
...
def downgrade() -> None:
    op.execute("""
        DROP INDEX IF EXISTS idx_events_site_session;

        ALTER TABLE events ADD COLUMN correlation_key VARCHAR(128);

        UPDATE events
        SET correlation_key = COALESCE(NULLIF(session_id, ''), NULLIF(visitor_id, ''))
